...
ln -s /mnt/beegfs/mf239628e/coat_color_evol/pseudo_ref_based/wwe_GG/chr_"$chromosome"/Lib_4_UAM_66285_"$chromosome".consensus.fa
ln -s /mnt/beegfs/mf239628e/coat_color_evol/pseudo_ref_based/wwe_GG/chr_"$chromosome"/Lib_4_UAM_83621_"$chromosome".consensus.fa
...
for i in $(ls *.frags.fa); do python /mnt/beegfs/mf239628e/coat_color_evol/pseudo_ref_based/gphocs/seq_clean_N.py $i 1000 40; done

#GET ID LIST FOR EACH FASTA AND SORT:
for i in $(ls clear*.fa); do cat $i | grep ">" | sort | uniq > ${i/.frags.fa/.ids.txt}; done

#FIND COMMON IDS:
comm -12 clear_Lib_6_JMG186_"$chromosome".ids.txt clear_Lib_4_UAM_66285_"$chromosome".ids.txt > comm1.txt
comm -12 comm1.txt clear_Lib_4_UAM_83621_"$chromosome".ids.txt > comm2.txt
...
python /mnt/beegfs/mf239628e/coat_color_evol/pseudo_ref_based/gphocs/combine_fastas.py clear_Lib_6_JMG186_"$chromosome".rpl.fa clear_Lib_4_UAM_66285_"$chromosome".rpl.fa clear_Lib_4_UAM_83621_"$chromosome".rpl.fa clear_JMG193_"$chromosome".rpl.fa > all_chr"$chromosome"_combined.fasta
...
python /mnt/beegfs/mf239628e/coat_color_evol/pseudo_ref_based/gphocs/gphocs_input.py all_chr"$chromosome"_combined_names.fasta "$chromosome" all_chr"$chromosome"_gphocs_input.txt
